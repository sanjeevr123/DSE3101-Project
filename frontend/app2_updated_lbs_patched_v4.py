...
def run_results(n, sell_payload, sell_geo, sell_pred, prefs_w, constraints, lbs_result):
    ...
    if not lbs_result or not lbs_result.get("ok"):
        return html.Div("Please complete Step 4: LBS details.", style=banner_warn), dash.no_update, None
...
@app.callback(
    ...
    State("recs_data", "data"),
    State("lbs_result", "data"),
    State("lbs_inputs", "data"),
    State("sell_pred", "data"),
    prevent_initial_call=True,
)
def render_comparison_modal(is_open, selected_indices, recs_data, lbs_result, lbs_inputs, sell_pred):
    ...
    if not selected_indices:
        return {
            ...
            "alignItems": "center",
            "overflow": "auto",
        }, html.Div("Please select at least one unit.", style={"color": "#ef4444"})

    # ---- Selected recommendation units ----
    selected_items = []
    for raw_idx in selected_indices:
        if isinstance(raw_idx, int) and recs_data and raw_idx < len(recs_data):
            rec = recs_data[raw_idx]
            selected_items.append({
                "label": f"Option #{raw_idx + 1}",
                "data": rec,
            })

    # ---- Ensure LBS result exists ----
    effective_lbs = lbs_result
    if not (effective_lbs and effective_lbs.get("ok")) and lbs_inputs:
        try:
            effective_lbs = compute_lbs_result(lbs_inputs, sell_pred)
        except:
            effective_lbs = None

    # ---- Inject LBS as Option #4 ----
    if effective_lbs and effective_lbs.get("ok"):
        selected_items.append({
            "label": "Option #4",
            "data": {
                "town": "Stay (LBS)",
                "rooms": "N/A",
                "buy_price": None,
                "cash_unlocked": float(effective_lbs.get("net_lbs", 0)),
                "dist_from_home_km": None,
                "nearest_healthcare": "N/A",
                "nearest_hawker": "N/A",
                "nearest_transport": "N/A",
                "nearest_nature": "N/A",
                "mrt_dist_km": None,
            },
        })
    ...
    def amenity(x):
        if isinstance(x, dict) and "name" in x and "dist_m" in x:
            return f"{x['name']} (~{x['dist_m']}m)"
        return "N/A"

    # ---- Build rows ----
    def build_row(label, key, formatter=lambda x: x):
        row = [html.Td(label, style={"fontWeight": "700", "padding": "10px"})]
        for item in selected_items:
            val = item["data"].get(key)
            row.append(html.Td(formatter(val), style={"textAlign": "center", "padding": "10px"}))
        return html.Tr(row)

    rows = [
        build_row("Town", "town"),
        build_row("Rooms", "rooms"),
        build_row("Buy Price (est.)", "buy_price", money),
        build_row("Cash Unlocked (est.)", "cash_unlocked", money),
        build_row("Distance from Your Flat", "dist_from_home_km", km),
        build_row("Nearest Healthcare", "nearest_healthcare", amenity),
        build_row("Nearest Hawker", "nearest_hawker"),
        build_row("Nearest Transport", "nearest_transport"),
        build_row("Nearest Park", "nearest_nature"),
        build_row("MRT Distance (approx)", "mrt_dist_km", km),
    ]

    table = html.Table(
        [
            html.Thead(html.Tr([
                html.Th(h, style={"padding": "12px", "fontWeight": "700", "borderBottom": "2px solid #0ea5e9"})
                for h in headers
            ])),
            html.Tbody(rows),
        ],
